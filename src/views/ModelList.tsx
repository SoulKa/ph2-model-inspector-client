...
function folderToTreeNodes( dir: ModelFolderObject, modelPath = "" ) {
    ...
    for (const [name, subdir] of Object.entries(dir)) {
        ...
        if (typeof subdir === "object") {
            nodes.push({
                ...
                childNodes: folderToTreeNodes(subdir, _modelPath),
                icon: "folder-close"
            });
            ...
            nodes.push({
                id: name,
                label: name.substring(0, name.length),
                nodeData: { modelPath: _modelPath, hasTexture: subdir },
...
export default function ModelList() {
    ...
    async function loadAllModels() {
        ...
        try {
            setBrowserNodes(folderToTreeNodes(await apiManager.getModels(apiManager.modelDirectory)));
    ...
    async function loadMapModels() {
        ...
        try {
            setMapNodes(folderToTreeNodes(await apiManager.getMapModels(mapName)));
    ...
    /**
     * Calback for dobule clicks in the model browser
     * @param node The clicked node (3D model or folder)
     */
    function browserNodeDoubleClicked( node: TreeNodeInfo<ModelObject> ) {
                ...
                <Card style={{ minWidth: "30em", margin: "1em", padding: 0, alignItems: "center", justifyContent: "center" }} elevation={Elevation.THREE} >
                    ...
                    <div style={{ overflowY: "scroll" }}>
                        <Tree
                            ...
                            onNodeDoubleClick={browserNodeDoubleClicked}
                        />
                    </div>
                </Card>
